...
from simu.simulation.contract.data.data_handler import DataHandler
from simu.simulation.contract.incentive.incentive_mechanism import IncentiveMechanism
from simu.simulation.contract.objects import Msg, SmartContract
...
class DefaultCollaborativeTrainer(CollaborativeTrainer):
    ...
    def add_data(self, msg: Msg, data, classification):
        ...
        cost, update_model = self.im.handle_add_data(
            msg.sender, msg.value, data, classification)
        self.data_handler.handle_add_data(
            msg.sender, cost, data, classification)
        if update_model:
    ...
    def refund(self, msg: Msg, data, classification, added_time: int):
        (claimable_amount, claimed_by_submitter, stored_data) = \
            self.data_handler.handle_refund(
                msg.sender, data, classification, added_time)
        ...
        self.data_handler.update_claimable_amount(
            msg.sender, stored_data, refund_amount)

    def report(self, msg: Msg, data, classification, added_time: int, original_author: str):
        claimed_by_reporter, stored_data = \
            self.data_handler.handle_report(
                msg.sender, data, classification, added_time, original_author)

        def prediction(): return self.model.predict(data)
        reward_amount = self.im.handle_report(
            msg.sender, stored_data, claimed_by_reporter, prediction)
        self.data_handler.update_claimable_amount(
            msg.sender, stored_data, reward_amount)
