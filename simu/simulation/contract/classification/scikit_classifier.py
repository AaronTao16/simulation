...
from simu.simulation.contract.classification.nearest_centroid_classifier import NearestCentroidClassifier
from simu.simulation.data.featuremapping.feature_index_mapper import FeatureIndexMapping
...
class SciKitClassifier(Classifier):
    ...
    def init_model(self, training_data, labels, save_model=False):
        ...
        self._logger.debug("training_data.shape: %s. dtype: %s",
                           training_data.shape, training_data.dtype)
        self._model.fit(training_data, labels)
        if save_model:
            self._logger.debug("Saving model to \"%s\".",
                               self._original_model_path)
            os.makedirs(os.path.dirname(
                self._original_model_path), exist_ok=True)
    ...
    def reset_model(self):
        assert self._model is not None, "The model has not been initialized yet."
        assert self._original_model_path.exists(
        ), "The model has not been saved. Perhaps saving was disabled."
        self._logger.debug("Loading model from \"%s\".",
                           self._original_model_path)
    ...
    def export(self,
        ...
        if isinstance(self._model, SGDClassifier) and self._model.loss == 'perceptron':
            ...
            if feature_index_mapping is not None:
                ...
                del model['weights']
                weights = {str(i): v for (i, v) in zip(
                    feature_index_mapping, weights) if v != 0}
                model['sparseWeights'] = weights
        elif isinstance(self._model, MultinomialNB):
            if classifications is None:
                classifications = list(
                    map(str, range(self._model.feature_count_.shape[1])))
            feature_counts = []
            ...
            for i, classification in enumerate(classifications):
                ...
                if feature_index_mapping is not None:
                    centroid = {str(i): v for (i, v) in zip(
                        feature_index_mapping, centroid) if v != 0}
